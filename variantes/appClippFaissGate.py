...
# -------------------------
# ✅ GATE: Bienvenida + Botón Iniciar (NO carga nada pesado hasta iniciar)
# -------------------------
...
if not st.session_state.started:
    st.title("👋 Bienvenido")
    st.write(
        "Esta app busca obras con **CLIP + FAISS** usando metadatos en **MySQL**.\n\n"
        "Para **iniciar** (cargar modelo e índice), presiona el botón."
    )
    ...
    with c1:
        if st.button("🚀 Iniciar", use_container_width=True, type="primary"):
            ...
            st.rerun()
    with c2:
        st.caption("Mientras no inicies, no se cargan FAISS/CLIP/MySQL para ahorrar recursos.")
...
with st.sidebar:
    ...
    # ✅ opcional: botón para “detener” y volver a la pantalla de bienvenida
    if st.button("⏸️ Detener (volver a inicio)", use_container_width=True):
        st.session_state.started = False
        st.rerun()
