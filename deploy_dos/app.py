    ...
    for i in range(n_cards):
        ...
        with c:
            ...
            img_ok = (
                    ...
                    and img_url.strip()
                    and img_url.strip().lower() not in {
...
# -------------------------
with tab_img:
    st.write("Sube una imagen (JPG/PNG/WebP) Funciona solo en Desktop")
    ...
    if submitted_img:
        if uploaded is None:
            ...
            st.image(img_query, caption="Imagen de consulta", width=180)
...
# -------------------------
with tab_meta:
    st.write("Buscador clásico por metadatos ")
...
with tab_map:
    ...
    with col_map:
        st.subheader("selecciona un punto")
        st.write("Indica la similitud visual-semántica entre las imágenes seleccionadas: cuanto más cerca estén en el espacio, mayor es su similitud. ")
